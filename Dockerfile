...
WORKDIR /template-mogodb-nestjs/src/app
...
COPY --from=development /template-mogodb-nestjs/src/app/ .
...
# docker build -t template-mogodb-nestjs-nest .
# docker run template-mogodb-nestjs-nest